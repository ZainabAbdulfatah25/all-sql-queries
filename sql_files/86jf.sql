...
BEGIN
  ...
  IF v_current_user_role IN ('admin', 'super_admin', 'state_admin') THEN
    ...
    RETURN v_new_user;
  ELSIF v_current_user_role IN ('organization', 'manager') AND p_role IN ('case_worker', 'manager', 'field_worker', 'organization') THEN
    -- Organizations/Managers can create non-admin users (specifically including case_worker)
    -- FORCE the organization assignment to match the creator's organization for security
    DECLARE
      v_org_id UUID;
      v_org_name TEXT;
    BEGIN
