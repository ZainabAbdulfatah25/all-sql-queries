...
-- We use a DYNAMIC DROP to catch any policy name we might have missed.
-- ----------------------------------------------------------------------------
...
DECLARE
  r RECORD;
BEGIN
  FOR r IN (SELECT policyname FROM pg_policies WHERE tablename = 'referrals' AND schemaname = 'public') LOOP
    EXECUTE 'DROP POLICY IF EXISTS "' || r.policyname || '" ON public.referrals';
