...
-- MIGRATION: Add Custom Notification Email Column
-- This allows users to set a preferred email for notifications that differs from their login email
...
begin
  ...
  if TG_TABLE_NAME = 'users' and TG_OP = 'INSERT' then
    if coalesce(new.notification_email_enabled, true) is true then
      -- Use custom notification email if set, otherwise login email
      recipient_email := coalesce(new.notification_email, new.email);
  ...
  if TG_TABLE_NAME = 'cases' and TG_OP = 'UPDATE' then
    -- Safe cast for assigned_to (which might be Text/OrgName) to UUID
    begin
      target_user_id := new.assigned_to::uuid;
    exception when others then
      target_user_id := null;
    end;

    -- If assigned_to wasn't a valid UUID, fallback to created_by (which is UUID)
    target_user_id := coalesce(target_user_id, new.created_by);
    
    if target_user_id is not null then
      -- SELECT notification_email along with other fields
      select coalesce(notification_email, email), name, notification_email_enabled, notification_case_updates 
  ...
  if TG_TABLE_NAME = 'referrals' and TG_OP = 'INSERT' then
    ...
    if dest_org_id is not null then
      -- Loop through ELIGIBLE users: Get Preferred Email
      for recipient_email, recipient_name in 
        select coalesce(notification_email, email), name from public.users 
        where organization_id = dest_org_id 
        and (role in ('organization', 'admin', 'state_admin', 'manager')) 
        and coalesce(notification_email_enabled, true) is true
        and coalesce(notification_referral_updates, true) is true
      loop
        
        param_sender_org_name := new.referred_from;
