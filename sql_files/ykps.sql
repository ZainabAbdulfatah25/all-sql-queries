...
-- Drop the function first to allow signature changes (e.g. adding p_description or p_user_type)
-- Drop the specific previous versions to avoid ambiguity
DROP FUNCTION IF EXISTS admin_create_user(uuid, text, text, text, text, text, text, text, uuid);
DROP FUNCTION IF EXISTS admin_create_user(uuid, text, text, text, text, text, text, text, uuid, text);
DROP FUNCTION IF EXISTS admin_create_user(uuid, text, text, text, text, text, text, text, uuid, text, text);
...
CREATE OR REPLACE FUNCTION admin_create_user(
  ...
  p_user_type TEXT DEFAULT 'individual' -- Default to individual if not specified
)
RETURNS users
LANGUAGE plpgsql
...
BEGIN
  ...
  IF v_current_user_role IN ('admin', 'super_admin', 'state_admin') THEN
    ...
    INSERT INTO users (id, email, name, role, phone, department, organization_name, organization_type, organization_id, description, user_type)
    VALUES (p_id, p_email, p_name, p_role, p_phone, p_department, p_organization_name, p_organization_type, p_organization_id, p_description, p_user_type)
  ...
  ELSIF v_current_user_role IN ('organization', 'manager') THEN 
    ...
    INSERT INTO users (id, email, name, role, phone, department, organization_name, organization_type, organization_id, description, user_type)
    VALUES (p_id, p_email, p_name, p_role, p_phone, p_department, v_org_name, p_organization_type, v_org_id, p_description, p_user_type)
    RETURNING * INTO v_new_user;
