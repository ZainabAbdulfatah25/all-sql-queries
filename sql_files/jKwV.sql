...
USING (
    ...
    referred_to = auth.uid()::text OR -- cast to text if referred_to is text, or just auth.uid() if uuid. Let's assume uuid but cast to be safe or check schema. Start with direct comparison if uuid. 
    -- Actually, if referred_to is UUID in DB, auth.uid() is UUID.
    referred_to = auth.uid()::text OR -- In case it's stored as text in this legacy schema
    referred_by = auth.uid()::text OR
