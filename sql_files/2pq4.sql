...
USING (
    ...
    referred_to::text = auth.uid()::text OR 
    referred_from::text = auth.uid()::text OR
    assigned_organization_id IN (
        SELECT id FROM organizations WHERE id::text = assigned_organization_id::text
    ) OR
    (auth.jwt() ->> 'user_metadata')::jsonb ->> 'role' IN ('admin', 'state_admin') OR
    assigned_organization_id::text = (SELECT organization_id::text FROM users WHERE id::text = auth.uid()::text)
);
...
USING (
    created_by::text = auth.uid()::text OR
    assigned_organization_id::text = (SELECT organization_id::text FROM users WHERE id::text = auth.uid()::text) OR
