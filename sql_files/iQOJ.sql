...
begin
  ...
  if TG_TABLE_NAME = 'cases' and TG_OP = 'UPDATE' and new.assigned_to is not null and (old.assigned_to is null or new.assigned_to != old.assigned_to) then
    select email, name into recipient_email, recipient_name from public.users where id = new.assigned_to;
...
drop trigger if exists on_case_assignment_email on public.cases;
create trigger on_case_assignment_email
